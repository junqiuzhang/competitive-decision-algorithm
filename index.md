...
基于混合策略的竞争决策算法是一种改进的竞争决策算法。在一般的竞争决策算法中，资源的分配是一种纯策略。但是，自然界中，资源的分配往往不是以简单的纯策略来进行的，而是以混合策略来进行的。竞争者往往不能独占资源，而是与其他竞争者共享资源。反映到选址问题中就是设施往往是以一定的概率服务顾客，而非确定服务某一顾客。
...
对于**算例 1**，我们限制其最大竞争步数为1次，此时每个设施占有的顾客：$F_{1}：\{ C_{2} \}$，$F_{2}：\{ C_{5} \}$，$F_{3}：\{ C_{3} \}$，$F_{4}：\{ C_{1},C_{4} \}$，其余设施占有的顾客为$\{\}$，总费用函数值（即目标函数值）为68
...
可以看到，基于混合策略的竞争决策算法在顾客数较多、设施数较少、容量限制也较小的算例中，得到比竞争决策算法的结果更好的比例较大，所以，基于混合策略的竞争决策算法适合求解顾客数相对设施总容量较多的软容量限制的选址问题。
