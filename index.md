...
所以，假设资源的分配策略是简单的纯策略是不够合理的，这里我们引入混合策略的概念，在竞争决策算法达到终止条件之后，不输出竞争结果，而是根据当前的竞争决策状态，按照混合策略再分配一次顾客，然后计算总费用函数的期望。如果使用混合策略分配得到的结果要优于纯策略分配的结果，则采用混合策略的分配结果作为输出的结果，否则就采用纯策略的结果作为输出结果。

具体的分配顾客混合策略就是，将顾客按照一定的概率分配给竞争力大于平均值的设施，分配的概率为：

$$
h(F_{i},C_{j}) = 
\left\{ 
\begin{matrix}
\frac{K_{ij}}{ \sum_{K_{ij} > \bar{K}}{K_{ij}}}
,K_{ij} > \bar{K}
\\
0
,K_{ij} <= \bar{K}
...
其中，$\bar{K} = \frac{ \sum_{i \in I}{K_{ij}}}{i_{max}}$
### 3.4.2 算例

**算例 2**

以竞争决策算法求解的**算例 1**为例，验证基于混合策略的竞争决策算法的有效性。因为**算例 1**已经求得了整数规划问题的最优解，使用基于混合策略的竞争决策算法必然不能找到更优的解，所以，我们限制竞争决策算法流程中第四步的竞争次数，使得竞争决策算法经过竞争阶段后输出的解不是最优解，然后采用混合策略继续分配，查看混合策略得到的解是否更优。
