...
在选址问题中，我们可以将设施看作竞争者，将顾客看作被设施竞争的资源，设施为顾客服务即竞争者获得了资源；同时，也可以将顾客看作竞争者，将设施看作被竞争的资源，顾客获得了设施的服务即竞争者获得了资源。为了便于理解，我们通常将设施看作竞争者，将顾客看作被设施竞争的资源。竞争决策状态为当前每个竞争者占有的资源情况，即设施为顾客服务的情况，也是选址问题整数规划模型的一个解。竞争决策状态的初始状态为每个竞争者都不占有资源，即没有任何设施为顾客服务。竞争决策状态的结束状态为每个竞争者最终占有的资源情况，即最终每个设施服务顾客的情况，通常也是竞争决策均衡状态。
...
而在选址问题中，决策函数就是如果重新分配某个资源给某个竞争者可以使总费用函数值目前已知的最优竞争决策状态下的总费用函数值，则重新分配资源。资源交换规则则是任意交换两个竞争者的全部资源，使竞争进入新的竞争决策状态。
...
**step 6：**
如果任一竞争者都不能根据竞争力函数和决策函数获得更多的资源，即达到竞争决策均衡，或者超过了最大竞争步数，则进入**step 6**；否则进入**step 4**

**step 7：**
关闭没有占有资源的竞争者，根据资源交换规则交换竞争者的资源，如果不能根据资源交换规则交换资源，使竞争进入非稳定的竞争决策状态，则进入**step 8**；否则进入**step 4**
...
相应地，因为软容量限制的设施选址问题中，设施可以开设多次，所以当设施容量已满，且仍要争夺资源时，就只需要重新开设一个设施，然后将资源分配给他即可。所以，软容量选址的设施选址问题中的算法流程如下：
...
**step 3：**
初始化：将剩余的资源按照次序分配给对资源来说竞争力函数最大的竞争者。如果该竞争者当前的容量已满，则打开一次该竞争者

**step 4：**
根据目前的竞争决策状态，更新每一个竞争者对每一个资源的竞争力函数值

**step 5：**
竞争者按照次序争夺资源，如果竞争者$F_{i}$对资源$C_{j}$的竞争力大于占有资源$C_{j}$的竞争者，且根据决策函数，可以将资源$C_{j}$分配给竞争者$F_{i}$，则将资源$C_{j}$分配给竞争者$F_{i}$。如果获得资源的竞争者当前的容量已满，则打开一次该竞争者
