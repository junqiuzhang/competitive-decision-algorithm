...
（5）对于$C_{2}$，更新$C_{2}$所在的列的竞争力函数矩阵值为：$H = [ 1/16; 1/18; 1/7; 1/6 ]$，当前服务$C_{2}$的设施$F_{4}$的竞争力最大，所以根据决策函数，不重新分配$C_{1}$

（6）对于$C_{3}$，更新$C_{1}$所在的列的竞争力函数矩阵值为：$H = [ 1/16; 1/18; 1/7; 1/6 ]$，当前服务$C_{1}$的设施$F_{4}$的竞争力最大，所以根据决策函数，不重新分配$C_{1}$

（7）对于$C_{4}$，更新$C_{1}$所在的列的竞争力函数矩阵值为：$H = [ 1/16; 1/18; 1/7; 1/6 ]$，当前服务$C_{1}$的设施$F_{4}$的竞争力最大，所以根据决策函数，不重新分配$C_{1}$

（8）对于$C_{5}$，更新$C_{1}$所在的列的竞争力函数矩阵值为：$H = [ 1/16; 1/18; 1/7; 1/6 ]$，当前服务$C_{1}$的设施$F_{4}$的竞争力最大，所以根据决策函数，不重新分配$C_{1}$
...
第八步：输出竞争结果，此时每个设施占有的顾客：$F_{1}：\{ C_{2},C_{5} \}$，$F_{2}：\{ C_{4},C_{5}\}$，其余设施占有的顾客为$\{\}$，总费用函数值（即目标函数值）为67
