...
\frac{1}{d_{ij}},\ \mathrm{设施已经确定建设且容量未满，或设施F_{i}已经服务顾客C_{j}} \\
\end{matrix} \right.\ \ $$

相应地，因为软容量限制的设施选址问题中，设施可以开设多次，所以当设施容量已满，且仍要争夺资源时，就只需要重新开设一个设施，然后将资源分配给他即可。所以，软容量选址的设施选址问题中的决策函数为：在满足新的竞争决策状态的总费用函数值更小的前提下，分配顾客给竞争力函数值最大的设施。
...
**step 4：**
按照次序争夺顾客：
**step 4.1：**根据目前的竞争决策状态，更新每一个设施对被争夺的顾客的竞争力函数值

**step 4.2：**
如果设施$F_{i_{1}}$对顾客$C_{j}$的竞争力大于服务顾客$C_{j}$的设施$F_{i_{2}}$，则根据决策函数，如果满足分配顾客$C_{j}$给设施$F_{i_{1}}$可以使总费用函数值减少，则将顾客$C_{j}$分配给设施$F_{i_{1}}$，并更新竞争决策状态

**step 5：**
如果任一设施都不能根据竞争力函数和决策函数获得更多的顾客，即达到竞争决策均衡，或者超过了最大竞争步数，则进入**step 6**；否则进入**step 4**

**step 6：**
关闭没有服务顾客的设施，根据资源交换规则交换设施的顾客，如果不能根据资源交换规则交换顾客，使竞争进入非稳定的竞争决策状态，或者超过了最大资源交换步数，则进入**step 7**；否则进入**step 4**
...
第一步：根据性质3，因为设施$F_{2}$的建设费用和为任何顾客服务的费用都大于设施$F_{1}$、$F_{3}$、$F_{4}$，即设施$F_{2}$相对设施$F_{1}$、$F_{3}$、$F_{4}$为劣势设施，所以在候选设施集合中关闭设施$F_{2}$，使$F_{2}$不参与竞争。根据性质2，设施$F_{1}$的建设费用最小，且对顾客$C_{1}$和顾客$C_{2}$的服务费用均为最小，所以$F_{1}$一定建设为顾客$C_{1}$和顾客$C_{2}$服务，所以固定顾客$C_{1}$、$C_{2}$，不参与竞争。此时设施服务顾客的情况：$F_{1}：\{ C_{1},C_{2}\}$，其余设施服务的顾客为$\{\}$，且设施$F_{2}$、顾客$C_{1}$、$C_{2}$不参与竞争。
...
第五步：设施按照次序争夺顾客：
（1）对于$C_{3}$，更新$C_{3}$所在的列的竞争力函数矩阵值为：$H = [ 1/22; 1/31; 1/24 ]$，根据决策函数，$F_{1}$的竞争力大于$F_{3}$，且从$F_{3}$中剥夺顾客$C_{3}$给$F_{1}$能够使总费用函数更优，故将顾客$C_{3}$分配给$F_{1}$，并更新竞争决策状态

（2）对于顾客$C_{4}$，更新$C_{4}$所在的列的竞争力函数矩阵值为：$H = [ 1/23; 1/31; 1/22 ]$，根据决策函数，$F_{3}$的竞争力小于$F_{4}$，故不重新分配顾客

（3）对于顾客$C_{5}$，更新$C_{5}$所在的列的竞争力函数矩阵值为：$H = [ 1/24; 1/31; 1/19 ]$，根据决策函数，$F_{4}$的竞争力大于$F_{3}$，且从$F_{3}$中剥夺顾客$C_{5}$给$F_{4}$能够使总费用函数更优，故将顾客$C_{4}$分配给$F_{4}$，并选取$F_{4}$服务的顾客中竞争力函数最小的一个顾客$C_{3}$作为交换分配给$F_{3}$，并更新竞争决策状态
...
（2）如果设施$F_{i_{1}}$的容量已满，则在设施$F_{i}$服务的顾客中选取一个竞争力函数最小的顾客作为交换分配给$F_{i_{2}}$，如果满足分配顾客给设施可以使总费用函数值减少，则将顾客$C_{j}$分配给设施$F_{i_{1}}$，并更新竞争决策状态。
...
但是，自然界中，资源的分配往往不是以简单的纯策略来进行决策的，而是受到很多随机因素的影响，包括内部随机因素和外部随机因素。例如在选址问题中，设施服务顾客的决策往往受到设施内部管理、经营因素的影响，以及外部经济、政治环境的影响，而存在一定的随机性，所以假设资源的分配策略是简单的纯策略是不够合理的。所以这里我们引入混合策略的概念，在竞争决策算法达到终止条件之后，采取混合策略是对竞争决策算法中策略的推广。
...
第一步：根据性质3，因为设施$F_{2}$的建设费用和为任何顾客服务的费用都大于设施$F_{1}$、$F_{3}$、$F_{4}$，即设施$F_{2}$相对设施$F_{1}$、$F_{3}$、$F_{4}$为劣势设施，所以在候选设施集合中关闭设施$F_{2}$，使$F_{2}$不参与竞争。根据性质2，设施$F_{1}$的建设费用最小，且对顾客$C_{1}$和顾客$C_{2}$的服务费用均为最小，所以$F_{1}$一定建设为顾客$C_{1}$和顾客$C_{2}$服务，所以关闭设施$F_{1}$，使$F_{1}$不参与竞争，固定顾客$C_{1}$、$C_{2}$，同样不参与竞争。此时设施服务顾客的情况：$F_{1}：\{ C_{1},C_{2}\}$，其余设施服务的顾客为$\{\}$，且设施$F_{1}$、$F_{2}$、顾客$C_{1}$、$C_{2}$不参与竞争。
...
（1）对于顾客$C_{3}$，更新竞争力函数矩阵中其所在的列的值，根据决策函数，$F_{3}$的竞争力小于$F_{4}$，故不重新分配顾客

（2）$F_{3}$争夺$C_{4}$，更新竞争力函数矩阵中其所在的列的值，根据决策函数，$F_{3}$的竞争力小于$F_{4}$，故不重新分配顾客

（3）$F_{4}$争夺顾客$C_{5}$，更新竞争力函数矩阵中其所在的列的值，根据决策函数，$F_{4}$的竞争力大于$F_{3}$，且从$F_{3}$中剥夺顾客$C_{5}$给$F_{4}$能够使总费用函数更优，故将顾客$C_{4}$分配给$F_{4}$，并选取$F_{4}$服务的顾客中竞争力函数最小的一个顾客$C_{3}$作为交换分配给$F_{3}$，并更新竞争决策状态

第六步：判断没有竞争者可以根据竞争力函数和决策函数获得更多的顾客

第七步：关闭没有服务顾客的设施，根据资源交换规则交换任意两个设施的任意两个顾客，重新竞争，均会竞争达到当前的竞争决策状态，故转到第八步。

第八步：输出竞争结果，此时每个设施占有的顾客：$F_{1}：\{ C_{1},C_{2}\}$，$F_{3}：\{ C_{3}\}$，$F_{4}：\{ C_{4},C_{5}\}$，其余设施占有的顾客为$\{\}$，总费用函数值即目标函数值为76
