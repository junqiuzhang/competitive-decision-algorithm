...
本文采用竞争决策算法对软容量限制的选址问题进行求解，并且对竞争决策算法进行了改进，引入了混合策略，优化了竞争决策算法。
...
**step 4.1：**
根据目前的竞争决策状态，更新每一个设施对被争夺的顾客的竞争力函数值
...
设候选设施集合$F_{i} \in F,i = 1,2,3,4,5$，顾客集合$C_{j} \in C,j = 1,2,3,4,5$，设每个候选设施的服务容量均为2，候选设施的建设费用：

$$H = \lbrack 13, 17, 10, 18, 10 \rbrack$$
...
第一步：根据性质3，因为设施$F_{2}$的建设费用和为任何顾客服务的费用都大于设施$F_{1}$、$F_{3}$、$F_{4}$，即设施$F_{2}$相对设施$F_{1}$、$F_{3}$、$F_{4}$为劣势设施，所以在候选设施集合中关闭设施$F_{2}$，使$F_{2}$不参与竞争。根据性质2，设施$F_{1}$对顾客$C_{1}$的服务费用加上设施$F_{1}$的建设费用小于其他顾客对顾客$C_{1}$的服务费用，所以$F_{1}$一定建设为顾客$C_{1}$服务，所以固定顾客$C_{1}$，不参与竞争。此时设施服务顾客的情况：$F_{1}：\{ C_{1}\}$，其余设施服务的顾客为$\{\}$，且设施$F_{2}$、顾客$C_{1}$不参与竞争。
...
（1）对于$C_{2}$，更新$C_{2}$所在的列的竞争力函数矩阵值为：$H = [ 1/7; 1/9; 1/26 ]$，当前服务$C_{2}$的设施$F_{1}$的竞争力最大，所以根据决策函数，不重新分配$C_{2}$

（2）对于$C_{3}$，更新$C_{3}$所在的列的竞争力函数矩阵值为：$H = [ 1/22; 1/9; 1/8 ]$，当前服务$C_{3}$的设施$F_{4}$的竞争力最大，所以根据决策函数，不重新分配$C_{3}$

（3）对于$C_{4}$，更新$C_{4}$所在的列的竞争力函数矩阵值为：$H = [ 1/23; 1/10; 1/6 ]$，当前服务$C_{4}$的设施$F_{4}$的竞争力最大，所以根据决策函数，不重新分配$C_{4}$

（4）对于$C_{5}$，更新$C_{5}$所在的列的竞争力函数矩阵值为：$H = [ 1/24; 1/28; 1/19 ]$，当前服务$C_{5}$的设施$F_{3}$的竞争力小于设施$F_{4}$，且从$F_{3}$中剥夺顾客$C_{5}$给$F_{4}$能够使总费用函数值更小，所以根据决策函数，从$F_{3}$中剥夺顾客$C_{5}$给$F_{4}$，并更新竞争决策状态，重新按照次序争夺顾客

（5）对于$C_{2}$，更新$C_{2}$所在的列的竞争力函数矩阵值为：$H = [ 1/7; 1/31; 1/10 ]$，当前服务$C_{2}$的设施$F_{1}$的竞争力最大，所以根据决策函数，不重新分配$C_{2}$

（6）对于$C_{3}$，更新$C_{3}$所在的列的竞争力函数矩阵值为：$H = [ 1/12; 1/31; 1/24 ]$，当前服务$C_{3}$的设施$F_{4}$的竞争力小于设施$F_{1}$，且从$F_{4}$中剥夺顾客$C_{3}$给$F_{1}$能够使总费用函数值更小，所以根据决策函数，从$F_{4}$中剥夺顾客$C_{3}$给$F_{1}$，并更新竞争决策状态，重新按照次序争夺顾客

第六步：此时，对于$C_{2}$、$C_{3}$、$C_{4}$、$C_{5}$，均没有竞争者可以根据竞争力函数和决策函数争夺到顾客，所以此时达到竞争决策均衡状态，将此时的竞争决策均衡状态记为最优竞争结果

第七步：关闭没有服务顾客的设施，根据资源交换规则交换任意两个设施的任意两个顾客，重新竞争，竞争结果均不优于最优竞争结果，故转到第八步。

第八步：输出竞争结果，此时每个设施占有的顾客：$F_{1}：\{ C_{1},C_{2},C_{3} \}$，$F_{4}：\{ C_{4},C_{5}\}$，其余设施占有的顾客为$\{\}$，总费用函数值（即目标函数值）为67

该解与使用精确解法分支定界法求得的解一致，验证了算法的有效性。

**其他算例**

为了更好的验证算法的有效性，我们采用JavaScript实现了软容量限制的设施选址问题的竞争决策算法，计算结果如下表所示：

设施数 | 顾客数 | 设施容量 | 竞争决策算法结果
---|---|---|---
5 | 5 | 2 | 79 
10 | 10 | 2 | 90
30 | 30 | 3 | 265
...
基于混合策略的竞争决策算法是一种改进的竞争决策算法。在一般的竞争决策算法中，资源的分配是一种纯策略。但是，自然界中，资源的分配往往不是以简单的纯策略来进行的，而是以混合策略来进行的。竞争者往往不能独占资源，而是与其他竞争者共享资源，并且资源的分配也呈现出一定的随机性。反应到选址问题中就是设施往往是以一定的概率服务顾客，而非确定服务某一顾客。

所以，假设资源的分配策略是简单的纯策略是不够合理的，这里我们引入混合策略的概念，在竞争决策算法达到终止条件之后，不输出竞争结果，而是根据当前的竞争决策状态，按照混合策略再分配一次顾客，然后计算总费用函数的期望。

其中，具体的分配顾客混合策略就是，将顾客按照一定的概率分配给竞争力大于平均值的设施，分配的概率为：

$$
h(F_{i},C_{j}) = 
\left\{ 
\begin{matrix}
\frac{K_{ij}}{ \sum_{i \in I}{K_{ij}}}
,K_{ij} > \frac{1}{F}
\\
0
,K_{ij} <= \frac{1}{F}
...
为了更好的验证改进后的算法的有效性，我们采用JavaScript分别实现了软容量限制的设施选址问题下的基于混合策略的竞争决策算法，结果如下表所示：

设施数 | 顾客数 | 设施容量 | 竞争决策算法结果 | 基于混合策略竞争决策算法结果
---|---|---|---|---
5 | 6 | 2 | 75 | 66.47
10 | 10 | 2 | 106 | 133.15
10 | 15 | 2 | 163 | 191.47
...
容易想到，在强容量限制的设施选址问题中，设施对顾客的服务成本和设施的建设成本越低，竞争力越大；所以，我们规定竞争力函数与设施的对顾客的服务费用和设施的建设费用成反比。进一步地，如果服务该顾客不需要重新建立设施，即设施未确定建设，则竞争力函数值仅与设施的对顾客的服务费用成反比；否则，设施已经确定建设，竞争力函数值就与设施的对顾客的服务费用与设施的建设费用之和成反比，故竞争力函数的表达式为：
...
因为在强容量限制的设施选址问题中，设施只能服务一定数量的顾客，且无法开设多次，所以强容量限制的设施选址问题中的决策函数为：在满足新的竞争决策状态的总费用函数值更小的前提下，分配顾客给竞争力函数值最大的设施，如果设施的容量未满，则直接分配顾客给设施；如果设施的容量已满，则在设施服务的顾客中选取一个竞争力函数最小的顾客作为交换，然后分配顾客给设施。而强容量限制的设施选址问题中，总费用函数和资源交换规则与软容量限制的相同。

### 4.3.2 竞争决策算法流程

相应地，因为强容量限制的设施选址问题中，设施的服务能力有限，所以在分配顾客给设施的时候还需要考虑设施是否达到服务能力上限，具体的算法流程如下：
...
同软容量限制的设施选址问题中的算例1，但是允许其中的设施多次开设。
...
（1）对于$C_{2}$，更新$C_{2}$所在的列的竞争力函数矩阵值为：$H = [ 1/7; 1/9; 1/10 ]$，当前服务$C_{2}$的设施$F_{1}$的竞争力最大，所以根据决策函数，不重新分配$C_{2}$

（2）对于$C_{3}$，更新$C_{3}$所在的列的竞争力函数矩阵值为：$H = [ 1/12; 1/9; 1/8 ]$，当前服务$C_{3}$的设施$F_{4}$的竞争力最大，所以根据决策函数，不重新分配$C_{3}$

（3）对于$C_{4}$，更新$C_{4}$所在的列的竞争力函数矩阵值为：$H = [ 1/13; 1/10; 1/6 ]$，当前服务$C_{4}$的设施$F_{4}$的竞争力最大，所以根据决策函数，不重新分配$C_{4}$

（4）对于$C_{5}$，更新$C_{5}$所在的列的竞争力函数矩阵值为：$H = [ 1/14; 1/28; 1/3 ]$，当前服务$C_{5}$的设施$F_{3}$的竞争力小于设施$F_{4}$，根据决策函数，从设施$F_{4}$中选取竞争力最小的顾客$C_{3}$换给设施$F_{3}$，计算得到交换$F_{3}$服务的顾客$C_{5}$和$F_{4}$服务的顾客$C_{3}$能够使总费用函数值更小，所以根据决策函数，交换$F_{3}$服务的顾客$C_{5}$和$F_{4}$服务的顾客$C_{3}$，并更新竞争决策状态，重新按照次序争夺顾客

（5）对于$C_{2}$，更新$C_{2}$所在的列的竞争力函数矩阵值为：$H = [ 1/7; 1/9; 1/10 ]$，当前服务$C_{2}$的设施$F_{1}$的竞争力最大，所以根据决策函数，不重新分配$C_{2}$

（6）对于$C_{3}$，更新$C_{3}$所在的列的竞争力函数矩阵值为：$H = [ 1/12; 1/9; 1/8 ]$，当前服务$C_{3}$的设施$F_{4}$的竞争力小于设施$F_{1}$，根据决策函数，需要从设施$F_{4}$中选取竞争力最小的顾客$C_{4}$换给设施$F_{3}$，计算得到交换$F_{3}$服务的顾客$C_{3}$和$F_{4}$服务的顾客$C_{4}$不能够使总费用函数值更小，所以根据决策函数，不重新分配$C_{3}$

第六步：此时，对于$C_{2}$、$C_{3}$、$C_{4}$、$C_{5}$，均没有竞争者可以根据竞争力函数和决策函数争夺到顾客，所以此时达到竞争决策均衡状态，将此时的竞争决策均衡状态记为最优竞争结果

第七步：关闭没有服务顾客的设施，根据资源交换规则交换任意两个设施的任意两个顾客，重新竞争，竞争结果均不优于最优竞争结果，故转到第八步。

第八步：输出竞争结果，此时每个设施占有的顾客：$F_{1}：\{ C_{1},C_{2}\}$，$F_{3}：\{ C_{3}\}$，$F_{4}：\{ C_{4},C_{5}\}$，其余设施占有的顾客为$\{\}$，总费用函数值（即目标函数值）为76

该解与使用精确解法分支定界法求得的解一致，验证了算法的有效性。

**其他算例**

为了更好的验证算法的有效性，我们采用JavaScript实现了强容量限制的设施选址问题的竞争决策算法，计算结果如下表所示：
