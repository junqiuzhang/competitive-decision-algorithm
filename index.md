...
因为设施选址决策受到设施的类型和数量、顾客的类型和数量、资金限制、竞争者的竞争力和数量、经济环境等多方面的因素的影响；设施选址决策会影响设施为顾客服务的成本、效率和质量等方面，进而影响设施服务顾客取得的利润；同时，选址的决策的影响是深远的，所以选址问题是重要的长期决策。所以，研究选址问题具有无论从经济方面来说还是社会方面来说都是十分有意义的$^{\lbrack 1\rbrack}$。

1909年，Weber研究了在平面上选取单个设施的位置使得设施与多个顾客的总距离最小的问题$^{\lbrack 2\rbrack}$，这被认为是选址问题研究的开始。1964年，Hakimi首次在论文中研究了$p -$中值问题和$p -$中心问题$^{\lbrack 3\rbrack}$，这篇论文大大推动了选址问题研究。自此之后，有关于选址问题研究的论文的数量急剧增多，选址问题研究也开始活跃了起来$^{\lbrack 1\rbrack}$。
...
本文在何永梅等人对软容量限制的设施选址问题下的竞争决策算法的研究基础上，重新归纳整理了软容量限制的设施选址问题下的竞争决策算法，并推广到强容量限制的设施选址问题中。同时，本文还对竞争决策算法进行了改进，提出了基于混合策略的竞争决策算法，即在竞争决策算法分配完成之后，再使用混合策略分配一次顾客，最终输出的结果取竞争决策算法的纯策略结果和混合策略的结果中的最优值。

本文通过NodeJS实现了两种问题下的竞争决策算法和基于混合策略的竞争决策算法，随机生成了大量算例来验证算法的有效性。结果表明，竞争决策算法在解决容量限制的选址问题时效果明显，能够在较短的时间内得到近似最优解，对于小规模的算例甚至能够得到全局最优解；而对于大规模的算例，在一定的计算资源限制下，可以得到较优的解。同时，通过引入混合策略，即使在计算资源限制下竞争决策算法仅仅得到了一般的解，再经过混合策略的再一次分配后，其混合策略解的期望有较高的概率优于纯策略解，因此我们可以通过一定的取整技巧得到比纯策略的竞争决策算法得到的解更优的解，这部分内容有待进一步的研究。并且，可以从算例结果数据中看到，本文提出的基于混合策略的竞争决策算法在处理顾客数相对设施数的较多的问题中表现优秀，而这类问题在现实中有很多应用场景，例如学校、医院等服务中心的选址，均属于顾客数远远大于设施数的选址问题。

此外，本文仅仅在竞争决策算法分配完成之后引入了混合策略，这其实并不是完全的引入混合策略，也就没有完全发挥出混合策略的优势。如果能够从算法一开始就引入混合策略，使每一次分配顾客都采用混合策略分配，这样就更加提高了算法的容错性。但是，由于混合策略是一种按概率的分配策略，所以在进行设施争夺顾客的迭代时，每种顾客分配情况都有一定概率发生，所以最终的分配情况会随迭代次数的增加而呈现指数倍的增加，这样算法显然是不能在可接受的时间和空间范围内得到结果的。所以，我们仅仅在竞争决策算法分配完成后引入了混合策略，这样就避免了迭代造成的分配情况指数递增的糟糕情况。而如何在竞争决策算法中完全引入混合策略也具有很大的讨论空间，有待于进一步的研究。
