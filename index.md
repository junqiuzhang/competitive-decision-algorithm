...
而在选址问题中，总费用函数就是目标函数；决策函数就是在满足新的竞争决策状态的总费用函数值更小的前提下，分配顾客给竞争力函数值最大的设施，如果竞争力函数值最大的设施的容量已满，则在设施服务的顾客中选取一个竞争力函数最小的顾客作为交换；资源交换规则就是交换任意两个设施服务的全部顾客，使竞争进入一个非稳定的竞争决策状态。
...
**step 3：**
将剩余的顾客按照次序分配给对顾客来说竞争力函数最大且容量未满的设施，并更新竞争决策状态。

**step 4：**
根据目前的竞争决策状态，更新每一个设施对每一个顾客的竞争力函数值

**step 5：**
设施按照次序争夺顾客，根据决策函数，如果设施$F_{i}$对顾客$C_{j}$的竞争力大于服务顾客$C_{j}$的设施，且分配顾客给设施可以使总费用函数值减少，则将顾客$C_{j}$分配给设施$F_{i}$，并更新竞争决策状态。
...
第一步：根据性质2，因为设施$F_{2}$的建设费用和为任何顾客服务的费用都大于设施$F_{1}$、$F_{3}$、$F_{4}$，即设施$F_{2}$相对设施$F_{1}$、$F_{3}$、$F_{4}$为劣势设施，所以在候选设施集合中关闭设施$F_{2}$，使$F_{2}$不参与竞争。根据性质1.1，设施$F_{1}$的建设费用最小，且对顾客$C_{1}$和顾客$C_{2}$的服务费用均为最小，所以$F_{1}$一定建设为顾客$C_{1}$和顾客$C_{2}$服务，所以关闭设施$F_{1}$，使$F_{1}$不参与竞争，固定顾客$C_{1}$、$C_{2}$，同样不参与竞争。此时设施服务顾客的情况：$F_{1}：\{ C_{1}\}$，其余设施服务的顾客为$\{\}$，且设施$F_{1}$、$F_{2}$、顾客$C_{1}$、$C_{2}$不参与竞争。
...
第三步：将剩余的顾客分配给竞争力函数最大的竞争者。得到新的竞争决策状态，此时设施服务顾客的情况：$F_{1}：\{ C_{1},C_{2}\}$，$F_{3}：\{ C_{5}\}$，$F_{4}：\{ C_{3},C_{4}\}$，其余设施服务的顾客为$\{\}$

第四步：更新所有设施的竞争力函数值，得到竞争力函数矩阵：
$$
\begin{matrix}
& C_{3} & C_{4} & C_{5} \\
\end{matrix}
...
（1）对于顾客$C_{3}$，竞争力函数最大的设施为$F_{3}$，根据决策函数，从设施$F_{4}$中剥夺顾客$C_{3}$给设施$F_{3}$能够使总费用函数更优，重新分配顾客$C_{3}$给设施$F_{3}$，并交换

（2）对于顾客$C_{4}$，竞争力函数最大的设施为$F_{4}$，根据决策函数，不重新分配顾客

（3）对于顾客$C_{5}$，竞争力函数最大的设施为$F_{4}$，根据决策函数，从设施$F_{3}$中剥夺顾客$C_{5}$给设施$F_{4}$能够使总费用函数更优，故将顾客$C_{5}$分配给设施$F_{4}$，并释放设施$F_{4}$的一个顾客$C_{3}$，使之重新分配

（4）对于顾客$C_{3}$，竞争力函数最大的设施为$F_{4}$，但是根据决策函数，虽然对于顾客$C_{3}$，竞争力函数最大的设施为$F_{4}$，但是从设施$F_{3}$中剥夺顾客$C_{3}$给设施$F_{4}$不能使总费用函数更优，所以不

第六步：判断没有竞争者可以根据竞争力函数和决策函数获得更多的资源

第七步：关闭没有服务顾客的设施，根据资源交换规则交换任意两个设施的全部资源，

第八步：输出竞争结果，此时每个设施占有的资源：$F_{1}：\{ C_{1},C_{2}\}$，$F_{3}：\{ C_{3}\}$，$F_{4}：\{ C_{4},C_{5}\}$，其余设施占有的资源为$\{\}$，目标函数值为76
...
相应地，因为软容量限制的设施选址问题中，设施可以开设多次，所以当设施容量已满，且仍要争夺资源时，就只需要重新开设一个设施，然后将资源分配给他即可。所以，软容量选址的设施选址问题中的决策函数为：当

### 4.3.2 竞争决策算法流程
**step 1：** 
设所有设施服务的顾客均为空集｛｝

根据性质1至3，绝对优势设施一定建设且服务的一定的顾客，相对劣势设施一定不服务顾客，所以关闭绝对优势设施和相对劣势设施，使之不参与竞争。

**step 2：** 
根据初始的竞争决策状态，计算每一个设施对每一个顾客的竞争力函数值

**step 3：**
将剩余的顾客按照次序分配给对顾客来说竞争力函数最大的设施，并更新竞争决策状态。
