...
**step 2：** 根据初始的竞争决策状态，计算每一个竞争者对每一个资源的竞争力函数值
...
**step 5：**
竞争者按照次序争夺资源，如果竞争者$F_{i}$对资源$C_{j}$的竞争力大于占有资源$C_{j}$的竞争者，且根据决策函数，可以将资源$C_{j}$分配给竞争者$F_{i}$，则将资源$C_{j}$分配给竞争者$F_{i}$。

每次分配资源后打开新分配到资源的竞争者，如果该竞争者容量已满，则打开相对于该竞争者为劣势的竞争者并更新竞争决策状态。

**step 6：**
如果任一竞争者都不能根据竞争力函数和决策函数获得更多的资源，即达到竞争决策均衡，或者超过了最大竞争步数，则进入**step 7**；否则进入**step 4**
...
（1）$F_{1}$争夺资源$C_{3}$，根据决策函数，从$F_{3}$中剥夺资源$C_{3}$给$F_{1}$不能使目标函数更优，故不重新分配资源

（2）$F_{3}$争夺资源$C_{2}$，根据决策函数，从$F_{1}$中剥夺资源$C_{2}$给$F_{3}$不能使目标函数更优，故不重新分配资源

（3）$F_{4}$争夺资源$C_{5}$，根据决策函数，从$F_{3}$中剥夺资源$C_{5}$给$F_{4}$能够使目标函数更优，故将资源$C_{5}$分配给$F_{4}$，更新竞争决策状态，并更新$F_{4}$的竞争力函数值为$\lbrack 1/36,1/31,1/31,1/32,1/28\rbrack$

第五步：判断没有竞争者可以根据竞争力函数和决策函数获得更多的资源
...
该模型同样使用竞争决策算法进行求解，算法流程与强容量限制的竞争决策算法相同，而其中的竞争力函数和决策函数略有不同，具体不同之处如下
...
（1）$F_{1}$争夺资源$C_{5}$，根据决策函数，从$F_{4}$中剥夺资源$C_{5}$给$F_{1}$步能使目标函数更优，故不重新分配资源

（2）$F_{3}$争夺资源$C_{2}$，根据决策函数，从$F_{1}$中剥夺资源$C_{2}$给$F_{3}$不能使目标函数更优，故不重新分配资源

（3）$F_{4}$争夺资源$C_{4}$，根据决策函数，从$F_{1}$中剥夺资源$C_{4}$给$F_{4}$能够使目标函数更优，故将资源$C_{4}$分配给$F_{4}$，更新竞争决策状态，并更新$F_{4}$的竞争力函数值为$\lbrack 1/31,1/26,1/24,1/22,1/19\rbrack$
